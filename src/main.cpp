 ...
 * main.cpp
 *
 * A heating timer with web control using Mongoose Application to pass back static web pages and handle rest requests and websocket
...
struct s_status {
	...
	double d_current_temperature = 0; // Current temperature in degrees celsius
	...
	if (state_changed) {
		...
		for (c = mgr->conns; c != NULL; c = c->next) {
			...
			mg_ws_printf(c, WEBSOCKET_OP_TEXT, 
				...
				g_status.timers[0][0], g_status.timers[0][1], g_status.timers[0][2], g_status.timer_targets[0],
				g_status.timers[1][0], g_status.timers[1][1], g_status.timers[1][2], g_status.timer_targets[1],
				g_status.timers[2][0], g_status.timers[2][1], g_status.timers[2][2], g_status.timer_targets[2],
				g_status.timers[3][0], g_status.timers[3][1], g_status.timers[3][2], g_status.timer_targets[3],
				g_status.timers[4][0], g_status.timers[4][1], g_status.timers[4][2], g_status.timer_targets[4],
				g_status.timers[5][0], g_status.timers[5][1], g_status.timers[5][2], g_status.timer_targets[5]
			);
		}
...
static void get_data() {
	...
	for (char i = 0; i < 6; i++) {
		...
		if (nvs->contains(key)) {
			...
			nvs->get_double(key, &v);
			g_status.timer_targets[i] = v;
		}
	}
	if (is_there_data)	{
...
int main(){
	...
	// Output pin
	gpio_init(GPIO_HEATING_TRIG); // Initialise the GPIO pin
    gpio_set_dir(GPIO_HEATING_TRIG, GPIO_OUT); // Set it as an output
